<script>
	...
	$("#staff_delegate").change(function() {
		...
		$.ajax({ 
			url: admin_url + 'hr_control/get_staff_role', 
			method: 'post', 
			data: formData, 
			contentType: false, 
			processData: false
	...
	function delete_contract_attachment(wrapper, id) {
		...
		if (confirm_delete()) {
			$.get(admin_url + 'hr_control/delete_hrm_contract_attachment_file/' + id, function (response) {
				if (response.success == true) {
		...
		if (changes !== null) {
			changes.forEach(([row, col, prop, oldValue, newValue]) => {
				if (col == 'rel_type' && oldValue != '') {
					requestGetJSON('hr_control/get_salary_allowance_value/' + oldValue).done(function (response) {						
