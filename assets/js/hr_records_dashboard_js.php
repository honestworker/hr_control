<script>
	...
	function staff_chart_by_fluctuate_according_to_seniority(id, value, title_c) {
		...
		requestGetJSON('hr_control/get_chart_senior_staff/' + sort_from+'/'+months_report).done(function (response) {
	...
	function report_by_staffs(id, value, title_c) {
		...
		requestGetJSON('hr_control/report_by_staffs').done(function (response) {
			//get data for hightchart
			Highcharts.setOptions({
				chart: {
					style: {
	...
	$(function() {
		...
		initDataTable(table_staff,admin_url + 'hr_control/table_reception', '','', '');
