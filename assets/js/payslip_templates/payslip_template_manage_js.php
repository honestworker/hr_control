...
initDataTable(payslip_template_table, admin_url+'hr_control/payslip_template_table',[0],[0], InvoiceServerParams, [0 ,'desc']);
...
function addPaySlipTemplate(form) {
	...
	if ((templates_name !== '' && payslip_columns.length > 0) ) {
		...
		$.post(admin_url + 'hr_control/payslip_template_checked', data).done(function(response) {
			response = JSON.parse(response);

			if (response.status === true || response.status == 'true') {
				var formData = $('#add_payslip_template').serialize();
        $.ajax({
            url: '<?= admin_url('hr_control/payslip_template') ?>',
            type: 'POST',
            dataType: 'json',
            data: formData,
        })
