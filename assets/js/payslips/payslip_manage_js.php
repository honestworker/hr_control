...
initDataTable(payslip_table, admin_url+'hr_control/payslip_table',[0],[0], InvoiceServerParams, [0 ,'desc']);
...
function new_payslip(){
    ...
    $('input[name="payslip_name"]').val('');
    var id = '';
    requestGetJSON('hr_control/get_payslip_template/' + id).done(function (response) {
...
function addPaySlip(form) {
    ...
    if (payslip_name !== '' && payslip_template_id.length > 0 && payslip_month != '' ) {
        ...
        $.post(admin_url + 'hr_control/payslip_checked', data).done(function(response) {
            response = JSON.parse(response);

            // Add pyslips template 
            if (response.status === true || response.status == 'true') {
                var formData = $('#add_payslip').serialize();
                $.ajax({
                    url: '<?= admin_url('hr_control/payslip') ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: formData,
                })
                .done(function(res) {
                    ...
                    $('.table-payslip_table').DataTable().ajax.reload();
                })
            } 
            else {
