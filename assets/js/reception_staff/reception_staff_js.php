	<script>	
		...
		function active_asset(el) {
			...
			$.post(admin_url+'hr_control/change_status_allocation_asset', data).done(function(response) {
		...
		function change_info_checklist(el) {
			...
			$.post(admin_url+'hr_control/change_status_checklist', data).done(function(response) {
				response = JSON.parse(response);
				if (response.success == true) { 
					change_progress();
				}
		...
		function save_add_asset() {
			...
			data.name = rtrim(t);
			$.post(admin_url+'hr_control/add_new_asset/<?php echo html_entity_decode($staff->staffid); ?>',data).done(function(response) {
				response = JSON.parse(response);
				if (response.success == true) {
		...
		function delete_asset(el) {
			'use strict';
			var val = $(el).data('id');
			$.post(admin_url+'hr_control/delete_asset/'+val+'/<?php echo html_entity_decode($staff->staffid); ?>').done(function(response) {
				response = JSON.parse(response);
				if (response.success == true) {
					$('#asset_list').html(response.data);
