<script>
	...
	$(function() {
		...
		initDataTable(table_resignation_procedures,admin_url + 'hr_control/table_resignation_procedures',[0], [0], StaffServerParams, [0, 'desc']);
	...
	function detail_checklist_staff(el) {
		...
		$.post(admin_url + 'hr_control/set_data_detail_staff_checklist_quit_work/'+staffid).done(function(response) {
	...
	function update_status_quit_work(el) {
		...
		$.post(admin_url + 'hr_control/update_status_quit_work', data).done(function(response) {
			response = JSON.parse(response);
	...
	$("#staffid").on('change', function() {
		...
		if (staff_id != '') {
			requestGetJSON('hr_control/get_staff_info_of_resignation_procedures/' + staff_id).done(function (response) {
