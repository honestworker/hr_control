...
class hr_control extends AdminController
{
    ...
    public function setting()
    {
        ...
        if (hr_payroll_get_status_modules('hr_control') && (get_hr_payroll_option('integrated_hrprofile') == 1)) {
        ...
        if ($data['group'] == '') {
            ...
            $data['hr_profile_active'] = hr_payroll_get_status_modules('hr_control');
    ...
    public function setting_incometax_rates()
    {
        if ($this->input->post()) {
            ...
            if (!$this->input->post('id')) {
                ...
                $message = ($mess) ? _l('hrp_updated_successfully') : _l('hrp_updated_failed');
    ...
    public function data_integration()
    {
        ...
        $mess = $this->hr_payroll_model->update_data_integration($this->input->post());
        $type = ($mess) ? 'success' : 'warning';
        $message = ($mess) ? _l('hrp_updated_successfully') : _l('hrp_updated_failed');
        set_alert($type, $message);
    ...
    {
        if ($this->input->post()) {
            $results = $this->hr_payroll_model->get_timesheet_type_for_setting($this->input->post());
            echo json_encode([
    ...
    public function add_attendance()
    {
        ...
        if ($this->input->post()) {
            ...
            if (isset($data)) {
                ...
                $message = ($success) ? _l('hrp_updated_successfully') : _l('hrp_updated_failed');
                set_alert($type, $message);
     ...
     * @return [type]
     */
    public function import_profile_employees_excel()
    {
        if (!has_permission('hrp_employee', '', 'create') && !has_permission('hrp_employee', '', 'edit') && !is_admin()) {
            access_denied('hrp_employee');
        }
    ...
    public function manage_deductions()
    {
        ...
        $data['title'] = _l('hrp_salary_deductions');
    ...
    public function add_manage_deductions()
    {
        ...
        if ($this->input->post()) {
            ...
            $type = ($success) ? 'success' : 'warning';
            $message = ($success) ? _l('updated_successfully') : _l('hrp_updated_failed');
            set_alert($type, $message);
    ...
    public function manage_commissions()
    {
        ...
        $data['title'] = _l('hrp_commission_manage');
    ...
    public function manage_insurances()
    {
        ...
        $data['title'] = _l('hrp_salary_insurances');
    ...
    public function get_payroll_column($id)
    {
        ...
        $method_option = $this->hr_payroll_model->get_list_payroll_column_method(['taking_method' => $taking_method]);

        //get function name html
        $functionName = ($payroll_column) ? $payroll_column->function_name : '';
        $function_name = $this->hr_payroll_model->get_list_payroll_column_function_name(['function_name' => $functionName]);
    ...
    public function payslip_update_status($id)
    {
        ...
        $message = ($result) ? _l('hrp_updated_successfully') : _l('hrp_updated_failed');
        set_alert($type, $message);
        redirect(admin_url('hr_control/payslip_manage'));
    }
    ...
    public function employees_copy()
    {
        ...
        if ($this->input->post()) {
            ...
            $message = ($results) ? _l('updated_successfully') : _l('hrp_updated_failed');
	...
	public function contract_type($id = '') {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=contract_type'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function allowance_type($id = '') {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=allowance_type'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function insurance_type() {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=insurrance'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function salary_form($id = '') {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=salary_type'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function add_procedure_form_manage() {
		...
		redirect(admin_url('hr_control/profile_setting?group=procedure_retire'));
	}
				...
				if (!empty($tmpFilePath) && $tmpFilePath != '') {
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						if ($total_row_false != 0) {
							...
							$objWriter->save(str_replace($filename, HR_CONTROL_ERROR . $filename, $filename));
						}
						$import_result = true;
						@delete_dir($tmpDir);
	...
	public function save_setting_reception_staff() {
		if ($this->input->post()) {
			...
			$message = _l('hr_updated_successfully');
			set_alert('success', $message);
			redirect(admin_url('hr_control/profile_setting?group=reception_staff'));
		}
	}
 ...
 */
	...
	public function update_staff_permission() {
		$data = $this->input->post();
		if ($data['id'] != '') {
			if (!$data['id'] == get_staff_user_id() && !is_admin() && !hr_profile_permissions('hr_control', '', 'edit')) {
				access_denied('hr_control');
			}
	...
	public function workplace($id = '') {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=workplace'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function hr_profile_update_permissions($id = '') {
		...
		redirect(admin_url('hr_control/profile_setting?group=hr_profile_permissions'));
	}
	...
	public function prefix_number() {
		...
		if ($data) {
			...
			redirect(admin_url('hr_control/profile_setting?group=prefix_number'));
		}
	}
	 ...
	 * @return [type]
	 */
	public function reset_profile_data() {
		if (!is_admin()) {
			access_denied('hr_control');
		}
		//delete Onboarding process
		...
		//delete sub folder contract
		foreach (glob(HR_CONTROL_CONTRACT_ATTACHMENTS_UPLOAD_FOLDER . '*') as $file) {
			$file_arr = explode("/", $file);
			$filename = array_pop($file_arr);

			if (is_dir($file)) {
				delete_dir(HR_CONTROL_CONTRACT_ATTACHMENTS_UPLOAD_FOLDER . $filename);
			}
		}

		//delete sub folder Q_A_ATTACHMENTS
		foreach (glob(HR_CONTROL_Q_A_ATTACHMENTS_UPLOAD_FOLDER . '*') as $file) {
			$file_arr = explode("/", $file);
			$filename = array_pop($file_arr);

			if (is_dir($file)) {
				delete_dir(HR_CONTROL_Q_A_ATTACHMENTS_UPLOAD_FOLDER . $filename);
			}
		}
		...
		set_alert('success', _l('reset_data_successful'));
		redirect(admin_url('hr_control/profile_setting?group=reset_data'));
	}
	...
	public function type_of_training($id = '') {
		if ($this->input->post()) {
			...
			if (!$this->input->post('id')) {
				...
				redirect(admin_url('hr_control/profile_setting?group=type_of_training'));
			} else {
				$id = $data['id'];
				unset($data['id']);
	...
	public function contract_sign($id) {
		...
		if ($this->input->post()) {
			if ($this->input->post('sign_by') == 'company') {
				process_digital_signature_image($this->input->post('signature', false), HR_CONTROL_CONTRACT_SIGN . $id);
				...
				hr_process_digital_signature_image($this->input->post('signature', false), HR_CONTROL_CONTRACT_SIGN . $id);
				$get_acceptance_info_array = get_acceptance_info_array();
	...
	public function contract_template($id = '') {
		if ($this->input->post()) {
			...
			if ($id == '') {
				...
				redirect(admin_url('hr_control/profile_setting?group=contract_template'));
			} else {
