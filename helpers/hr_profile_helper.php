...
function get_hr_profile_upload_path_by_type($type)
{
	...
	switch ($type) {
		case 'staff_contract':
			$path = HR_CONTROL_CONTRACT_ATTACHMENTS_UPLOAD_FOLDER;
			break;

		case 'job_position':
			$path = HR_CONTROL_JOB_POSIITON_ATTACHMENTS_UPLOAD_FOLDER;
			break;

		case 'kb_article_files':
			$path = HR_CONTROL_Q_A_ATTACHMENTS_UPLOAD_FOLDER;
			break;
		
		case 'att_files':
			$path = HR_CONTROL_FILE_ATTACHMENTS_UPLOAD_FOLDER;
			break;
	}
...
{
	return app_pdf('contract',  module_dir_path(HR_CONTROL_MODULE_NAME, 'libraries/pdf/Hr_contract_pdf'), $contract);
}
