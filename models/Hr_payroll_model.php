...
class Hr_payroll_model extends App_Model {
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_income_tax_rates', ['id' => $id])->row();
        }
    	return $this->db->get(db_prefix(). 'hrp_income_tax_rates')->result_array();
    }
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_income_tax_rebates', ['id' => $id])->row();
        }
		return $this->db->get(db_prefix(). 'hrp_income_tax_rebates')->result_array();
    }
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_earnings_list', ['id' => $id])->row();
        }
        return $this->db->get(db_prefix() . 'hrp_earnings_list')->result_array();
    }
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_salary_deductions_list', ['id' => $id])->row();
        }
		return $this->db->get(db_prefix() . 'hrp_salary_deductions_list')->result_array();
    }
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_insurance_list', ['id' => $id])->row();
        }
	...
	{
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_company_contributions_list', ['id' => $id])->row();
        }
        return $this->db->get(db_prefix() . 'hrp_company_contributions_list')->result_array();
    }
	...
	public function hr_records_get_earnings_list($id = false) {
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_earnings_list_hr_records', ['id' => $id])->row();
        }
        return $this->db->get(db_prefix() . 'hrp_earnings_list_hr_records')->result_array();
    }
    ...
    */
	...
	public function get_hrp_payroll_columns($id = false) {
		...
		if ($id == false) {
			return $this->db->order_by('order_display', 'asc')->get(db_prefix(). 'hrp_payroll_columns')->result_array();
		}
	}
	...
	{
	    if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrp_payslip_details', ['id' => $id])->row();
        }
	...
	public function employees_copy($data)
	{
		...
		if (count($employees) > 0) {
			...
			$message = ($affectedRows > 0) ? _l('hrp_added_successfully') : _l('hrp_add_failed');
