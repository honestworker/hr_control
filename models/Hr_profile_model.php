...
class Hr_profile_model extends App_Model
{
    ...
    public function delete_hr_profile_staff_attachment($attachment_id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(HR_CONTROL_FILE_ATTACHMENTS_UPLOAD_FOLDER . '/' . $attachment->rel_id . '/' . $attachment->file_name);
            }
            ...
            if (is_dir(HR_CONTROL_FILE_ATTACHMENTS_UPLOAD_FOLDER . '/' . $attachment->rel_id)) {
                $other_attachments = list_files(HR_CONTROL_FILE_ATTACHMENTS_UPLOAD_FOLDER . '/' . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    delete_dir(HR_CONTROL_FILE_ATTACHMENTS_UPLOAD_FOLDER . '/' . $attachment->rel_id);
                }
            }
        }
        return $deleted;
    ...
    public function report_by_leave_statistics()
    {
        ...
        if ($months_report != '') {
            if (is_numeric($months_report)) {
                ...
                $custom_date_select = '(hrl.start_time BETWEEN "' . date('Y-m-d', strtotime(date('Y-01-01'))) . '" AND "' . date('Y-m-d', strtotime(date('Y-12-31'))) . '")';
            } elseif ($months_report == 'last_year') {
                $custom_date_select = '(hrl.start_time BETWEEN "' . date('Y-m-d', strtotime(date(date('Y', strtotime('last year')) . '-01-01'))) . '" AND "' . date('Y-m-d', strtotime(date(date('Y', strtotime('last year')) . '-12-31'))) . '")';
    ...
    public function contract_clear_signature($id)
    {
        ...
        if ($contract) {
            ...
            if (!empty($contract->signature)) {
                unlink(HR_CONTROL_CONTRACT_SIGN . $contract->id_contract . '/' . $contract->signature);
            }
            ...
            if ($contract) {
                ...
                if ($for_editor == false) {
                    ...
                    if (strlen($contract->staff_signature) > 0) {
                        $contract->content .= '<img src="' . site_url('download/preview_image?path=' . protected_file_url_by_path(HR_CONTROL_CONTRACT_SIGN . $contract->id_contract . '/' . $contract->staff_signature)) . '" class="img-responsive" alt="">';
                    } else {
                    ...
                    if (strlen($contract->signature) > 0) {
                        $contract->content .= '<img src="' . site_url('download/preview_image?path=' . protected_file_url_by_path(HR_CONTROL_CONTRACT_SIGN . $contract->id_contract . '/' . $contract->signature)) . '" class="img-responsive" alt="">';
                    } else {
                        $contract->content .= '<img src="" class="img-responsive" alt="">';
                    }
        ...
        if (is_numeric($id)) {
            ...
            if ($contract) {
                if ($for_editor == false) {
					...
					<tr>';
                    if (strlen($contract->staff_signature) > 0) {
                        $contract->content .= '<td  width="50%" class="text-left"><img src="' . site_url('download/preview_image?path=' . protected_file_url_by_path(HR_CONTROL_CONTRACT_SIGN . $contract->id_contract . '/' . $contract->staff_signature)) . '" class="img-responsive" alt=""></td>';
                    } else {
                        $contract->content .= '<td  width="50%" class="text-left"><img src="" class="img-responsive" alt=""></td>';
                    }

                    if (strlen($contract->signature) > 0) {
                        $contract->content .= '<td  width="50%" class="text_right"><img src="' . site_url('download/preview_image?path=' . protected_file_url_by_path(HR_CONTROL_CONTRACT_SIGN . $contract->id_contract . '/' . $contract->signature)) . '" class="img-responsive" alt=""></td>
					</tr>';
                    } else {
                    ...
                    $contract->content .= '<link href="' . module_dir_url(HR_CONTROL_MODULE_NAME, 'assets/css/pdf_style.css') . '"  rel="stylesheet" type="text/css" />';
                }
            }
    ...
    public function update_hr_staff_contract_content($id, $staffid)
    {
