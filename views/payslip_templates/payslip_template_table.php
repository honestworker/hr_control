...
$sIndexColumn = 'id';
$sTable = db_prefix() . 'hr_payslip_templates';
...
foreach ($rResult as $aRow) {
	$row = [];
		...
		//load by staff
		if(!is_admin() && !has_permission('hr_payslip_template','','view') && $aRow['staff_id_created'] != get_staff_user_id()){
      	//View own
			$staffids = $this->ci->hr_control_model->payslip_template_get_staffid($aRow['department_id'], $aRow['role_employees'], $aRow['staff_employees'], $aRow['except_staff']);
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id'){
			...
			$code = '<a href="' . admin_url('hr_control/view_payslip_templates_detail/' . $aRow['id']) . '">' . $aRow['templates_name'] . '</a>';
			$code .= '<div class="row-options">';

			$code .= '<a href="' . admin_url('hr_control/view_payslip_templates_detail/' . $aRow['id']) . '" >' . _l('view_detail') . '</a>';

			if (has_permission('hr_payslip_template', '', 'edit') || is_admin()) {

				$code .= ' | <a href="#" onclick="edit_payslip_template(this, '.$aRow['id'] .'); return false;"  >' . _l('edit') . '</a>';
			}
			if (has_permission('hr_payslip_template', '', 'delete') || is_admin()) {
				$code .= ' | <a href="' . admin_url('hr_control/delete_payslip_template/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
			}
			$code .= '</div>';
